...
> 单一大脑（默认 Sonnet 4.6，可配置升 Opus）+ 多工具；**控制流是大脑的推理，不是状态机**。**用户深度参与、但省心省力**：首稿出候选交用户选基准，判图/打分已删除（D39），多视角靠进化式参考链保持一致。

## 现状
- ✅ **Phase 0-4 + 架构收敛 D39** Loop Agent 全链路（澄清 → 方案 → 布局确认 → 首稿候选 → 用户选基准 → 按需多视角/局部精修 → 交付）· projectId 隔离 · 多模态上传 · ASR 语音
...
- ✅ **首稿候选机制**：final 首稿默认 `n=2 / quality=medium` 并发生成，两张候选先显示在对话中，用户点选后才进入正式资产库并写入 `baseAssetId`（选基准后自动承接 loop，提示下一步）
...
- ✅ **Run 记录 + 代码层流程守卫**：每轮 `/api/agent` 生成 runId，记录 step/tool/deliverable；prompt-writer 等隐藏模型调用也写 usage；final render 必须已有 spec.identity 且布局已确认或明确跳过
- ✅ **历史上下文瘦身 + 成本估算**：对话 UI 与项目状态完整保存，但每轮发给模型前会压缩历史工具输出和旧文本；`estimate_cost` 走 DeepSeek V4 Flash 低成本解释余额消耗
- ✅ **质量闭环基础设施**：展台规则引擎单测（Vitest）+ 10 个真实案例回归集（`fixtures/booth-cases`，不调模型）+ 几何单一来源 `geometry.ts` + 生图锁定 gpt-image-2/fal（D39）+ 生成耗时/供应商沉淀进 asset/run
...
- **经 Vercel AI Gateway**：脑默认 `anthropic/claude-sonnet-4.6`（`RHEMOS_BRAIN_MODEL` 可切 Opus）· 写图 prompt 默认 `anthropic/claude-opus-4.8` · 成本解释/语音清理 `deepseek/deepseek-v4-flash`。
...
> 生图已锁定 **gpt-image-2 / fal**（唯一指定模型 + 唯一渠道，D39）；`image-providers.ts` 是 fal 锁定的薄封装——以后要换/加渠道只在这一处改。见 [DECISIONS](docs/DECISIONS.md) D29 / D39。
...
#   AI_GATEWAY_API_KEY=...   必需，路由经 Gateway 的模型（脑/写prompt/成本估算/语音清理）
#   FAL_API_KEY=...          必需，gpt-image-2 文生图 + 图编辑经 fal.ai（唯一指定生图模型 + 唯一渠道）
...
## 红线
模型多来源（脑/写prompt/成本估算/语音清理经 Gateway · gpt-image-2 经 fal.ai 唯一渠道 · ASR 经 DashScope 直连）· 首稿候选必须由用户选基准后才进入资产库 · final render 不绕过 spec/layout 决策 · 外轮廓未说明异形则严格矩形 · 品牌无素材只占位 · `.env.local` / `.data/` 不入库 · 中文对话/注释/commit。
