...
API     src/app/api/agent/route.ts —— streamText(orchestrator)；附件预处理 + 历史工具/旧文本上下文瘦身 + tool 入参兜底
   │   其余：assets/[id]（图片读出）· projects/(+[id]、[id]/state)（列表/删除/状态）· asr（语音）
Agent 核心  src/agent/orchestrator.ts —— 单脑默认 Sonnet 4.6（可配置）+ stopWhen + 工具集 + Run 记录
   │   system = src/agent/system-prompt.ts（PREAMBLE + 决策型 skills + 2 rubrics）
   ├── 工具  src/tools/*（Zod）；render 拆成 src/tools/render/{context,candidates,views-chain}.ts（仅编排，D39）
   ├── 几何  src/lib/geometry.ts（单一来源：EDGES/贴边/面积/footprint，booth-rules·cad·present-choices 共用，D39）
...
模型层  src/models/gateway.ts + image-providers.ts —— 多来源：脑/写prompt/成本估算/语音清理经 Gateway · gpt-image-2 经 fal.ai（唯一渠道，D39）· ASR 经 DashScope 直连
存储层  src/lib/storage.ts —— 本地 .data/projects/<id>/（state.json + assets/*.png）+ 写锁
```

## 一次请求的流
用户消息 → `/api/agent` 创建 `Run` → 附件引用按需还原/提取，历史工具输出和旧文本压缩成摘要（UI 历史不变）→ 大脑循环：`read_project_state` →（需拍板）`present_choices` 出卡片 + 俯视布局草图（布局骨架一次只问一个，用户选完再重排下一问）→ `update_brief` 落已确认事实 → `update_spec` 写方案（含 **identity 身份锁定 + footprint 外轮廓硬规则**，未明确异形时默认严格矩形，narrative 引用 layout 对象 ID）→ `present_layout` 写入对象级 `layout.status=pending` 并显示“打开编辑器 / 按原方案出图”的确认入口 → 用户确认截图 + 对象级坐标（`layout.status=confirmed + planAssetId + proposal`）或跳过（`layout.status=skipped`）→ `render(views=[], n=2)` 把截图作为 floor_plan 参考，并把 `layout.proposal` 编译成 **Rhemos CAD v1** 机读硬锁注入最终 prompt，生成 **candidate-set 首稿候选**（候选图片落盘但不进入正式 assets）→ 用户在对话里点“选为基准” → `/api/projects/:id/assets/:assetId/promote` 把该候选加入正式资产库并写 `baseAssetId`（前端自动发人话承接 loop）→ 用户明确要求多视角/俯视/深化时，`render(views=[...])` 从 `baseAssetId` 走**进化式参考链**（串行累积、无门控，D39）继续 → 必要时 `revise_asset` 局部精修 → `task_complete` → Run 记录 step/tool/deliverable/status/隐藏模型 usage，全程流式回前端。

## Loop Agent 机制（`src/agent/orchestrator.ts`）
- `model` = `MODEL_IDS.brain`，默认 `anthropic/claude-sonnet-4.6`；可用 `RHEMOS_BRAIN_MODEL=anthropic/claude-opus-4.8` 切回 Opus。
...
- **Run 记录**：每轮 `/api/agent` 创建 `run-...`，写到 `.data/projects/<id>/runs/<runId>.json`，项目状态保留最近 30 条摘要；`onStepFinish` 记录 step/tool 摘要，`render`/`revise_asset` 记录 Deliverable，`prompt-writer` 等隐藏模型调用也写 usage，`onFinish/onError/onAbort` 收口状态。
- **模型分档现状**：主脑默认 Sonnet 4.6 控成本；prompt-writer 默认 Opus 4.8 保留写图/世界知识优势；成本解释走 DeepSeek V4 Flash。均可用 `RHEMOS_*_MODEL` 环境变量覆盖。**判图/打分已删除（D39）**，无 inspect 档。

## 工具注册表（`src/tools/*`，Zod schema）
| 工具 | 作用 | 模型 | 关键 I/O |
|---|---|---|---|
| `read_project_state` | 读 brief / spec / layout / `baseAssetId` / 资产**摘要** / 最近 Run（瘦身：不回长 prompt） | — | → {brief, spec, layout, baseAssetId, recentRuns, assets[]摘要} |
...
| `analyze_reference` | 看参考图抽设计语言 | 当前 brain 模型(vision) | imageUrl → analysis |
...
| `present_layout` | **方案定稿后推布局**：规范化对象级 layout，写 `layout.status=pending`，前端显示“打开编辑器 / 按原方案出图”的确认入口 | — | intro / layout → 透传，用户明确点击后进入 LayoutEditor |
| `render` | **唯一生图入口**：给**中文意图**，内部 prompt-writer 写英文 prompt；final 模式硬要求 spec.identity + layout confirmed/skipped；按平面图出图时同时使用 reference PNG 和 **Rhemos CAD v1** 机读布局硬锁（坐标系/开口边/footprint/对象 bbox+layer+shape+height+facing+material）；默认输出 `candidate-set` 首稿候选，用户选 `baseAssetId` 后才允许 views 多视角深化（**进化式参考链·无门控**，D39）；拆为 `tools/render/{context,candidates,views-chain}.ts` | gpt-image-2(fal) + **Opus prompt-writer** | intent / views / planAssetId / mode / n / quality → **Deliverable** |
| `revise_asset` | **参考图局部精修**：只改一处其余不变；给**中文** fix，内部 prompt-writer 翻英文 | gpt-image-2(fal) + **Opus prompt-writer** | parentAssetId / fix(中文) → **Deliverable** |
| `estimate_cost` | 读取 run usage 做文本模型成本归因，并用 DeepSeek V4 Flash 低成本解释 | DeepSeek V4 Flash | scope / limit → estimate + explanation |
| `task_complete` | 声明完成、退出循环 | — | summary / delivered / gaps |

## 模型矩阵（`src/models/gateway.ts` + `src/models/image-providers.ts`）
| 角色 | 模型 | 经由 |
|---|---|---|
| 脑（对话+工程） | 默认 `anthropic/claude-sonnet-4.6`，`RHEMOS_BRAIN_MODEL` 可覆盖 | Gateway（`gateway.languageModel`）|
| 文生图 | `openai/gpt-image-2` | **fal.ai**（`fal.run/openai/gpt-image-2`，`falTextToImage`）；本地测试期默认 quality medium（fal API 速度 ≠ ChatGPT 体感）|
| 参考条件化 / 编辑 | `openai/gpt-image-2` | **fal `…/edit`**（`falEditFromRefs`，base64 data URI 多图参考）。**唯一 gpt-image-2/fal（D39），无 Gemini fallback**；历史 D27：gpt-image-2 经 Gateway 图输入不通，现走 fal。|
| 写 prompt(worker) | 默认 `anthropic/claude-opus-4.8`，`RHEMOS_PROMPT_MODEL` 可覆盖 | Gateway；工具内子 agent |
| 成本解释 / 语音清理 | 默认 `deepseek/deepseek-v4-flash` | Gateway（低成本解释余额消耗；ASR 后处理）|
| ASR | DashScope `fun-asr-realtime` | 直连（非 Gateway 例外，China region）|
| 视频 | —— | 砍 |

> **多来源例外**：gpt-image-2 经 **fal.ai**、ASR 经 **DashScope** 直连，其余经 Gateway。`image-providers.ts` 把 `textToImage`/`editFromRefs` 收口为 **fal 锁定薄封装**（生图唯一 gpt-image-2/fal，D39）——以后要换/加渠道只在这一处改，详见 [DECISIONS](DECISIONS.md) D29/D39。
...
**左**项目面板（列表 / 切换 / 新建 / 删除 / 高亮）｜ **中**对话（`useChat`；文字 / 语音 / 上传；上传先走 `/api/projects/:id/attachments` 资产化，再以轻量 FileUIPart 发送；assistant 走 **react-markdown** 渲染；需拍板时 `present_choices` → **ChoiceCards** 可点卡片 + **FloorPlan** 数据驱动俯视草图，布局类只显示当前一步；方案后 `present_layout` 显示布局确认入口，用户可打开 **LayoutEditor**(react-konva) 对象库/拖拽/缩放/属性面板/撤销重做/规则提示或直接跳过 → 截图存 reference，并把拖拽后的对象坐标写回 `layout.proposal` → 首稿 `candidate-set` 两张候选；候选图按钮“选为基准”会调用 promote API，选中后才进右侧资产画廊；工具过程仅"调试"可见）｜ **右**资产画廊。**对话持久化**：流式增量存盘（`/api/projects/:id/messages`，`pidRef` 防串项目），切项目 / 刷新不丢。`src/app/page.tsx` 仅 `redirect('/projects/default')`。
...
- **生图锁定 fal**（D39）：`image-providers.ts` 是 fal 锁定薄封装（`textToImage`/`editFromRefs` 直连 fal gpt-image-2），导出常量 `IMAGE_PROVIDER='fal'`/`IMAGE_MODEL` 供元数据；已删可插拔脚手架与 `IMAGE_PROVIDER` env。fal 下载有 SSRF 白名单 + 超时/重试 + AbortSignal。
...
src/
  app/
    ...
    api/agent/route.ts             Orchestrator 入口（streamText + 附件预处理 + 历史工具/旧文本上下文瘦身 + 入参兜底）
  ...
  agent/
    orchestrator.ts                工具集 + stopWhen + 预算（用 brain() 句柄）
    system-prompt.ts               装配 system prompt（PREAMBLE + 知识层）
    prompt-writer.ts               写图 prompt 子 agent（中文意图 → 英文 prompt，带执行型知识）
  tools/                           Zod 工具（见上表：render 唯一生图入口 / present_choices / estimate_cost 等）
    render.ts + render/{context,candidates,views-chain}.ts  render 拆分：仅编排 + 门控准备/候选三分支/进化链（D39）
  models/gateway.ts                模型句柄 + fal 封装（Gateway / fal.ai / DashScope 多来源；fal SSRF/超时/重试/AbortSignal）
  models/image-providers.ts        生图 provider 层（fal 锁定薄封装，D39：textToImage/editFromRefs 直连 fal + 导出 IMAGE_PROVIDER/IMAGE_MODEL）
  ...
  lib/
    ...
    cad.ts                         Rhemos CAD v1：BoothLayout → 机读 CAD 文档 / 生图硬锁 + buildFootprintLock（footprint 硬规则单一来源）
    geometry.ts                    几何单一来源（D39：EDGES/贴边 touchesEdge·hugsEdge/面积/closedEdges/openingRelation；booth-rules·cad·present-choices 共用）
    cost-estimate.ts               run usage 成本归因（纯函数）
    booth-rules.ts                 最小展台规则引擎（纯函数 15 条，present-layout/choices/render 接入）
    booth-rules.test.ts            booth-rules 单测（Vitest，含顶部/detail 不计地面占地 + openingRelation）
    ...
    cad.test.ts                    CAD 文档转换与 prompt lock 单测
    cost-estimate.test.ts          成本估算聚合单测
