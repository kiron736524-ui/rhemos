...
Agent 核心  src/agent/orchestrator.ts —— 单脑默认 Sonnet 4.6（可配置）+ stopWhen + 工具集 + Run 记录
   ...
   ├── 工具  src/tools/*（Zod）
   ├── 判图  src/agent/inspect.ts（generateObject + 默认 Sonnet 4.6，可配置升 Opus）
...
## 一次请求的流
用户消息 → `/api/agent` 创建 `Run` → 附件引用按需还原/提取，历史工具输出和旧文本压缩成摘要（UI 历史不变）→ 大脑循环：`read_project_state` →（需拍板）`present_choices` 出卡片 + 俯视布局草图（布局骨架一次只问一个，用户选完再重排下一问）→ `update_brief` 落已确认事实 → `update_spec` 写方案（含 **identity 身份锁定 + footprint 外轮廓硬规则**，未明确异形时默认严格矩形，narrative 引用 layout 对象 ID）→ `present_layout` 写入对象级 `layout.status=pending` 并显示“打开编辑器 / 按原方案出图”的确认入口 → 用户确认截图 + 对象级坐标（`layout.status=confirmed + planAssetId + proposal`）或跳过（`layout.status=skipped`）→ `render(views=[], n=2, autoCheck=false)` 把截图作为 floor_plan 参考，并把 `layout.proposal` 编译成 **Rhemos CAD v1** 机读硬锁注入最终 prompt，生成 **candidate-set 首稿候选**（候选图片落盘但不进入正式 assets）→ 用户在对话里点“选为基准” → `/api/projects/:id/assets/:assetId/promote` 把该候选加入正式资产库并写 `baseAssetId` → 用户明确要求多视角/俯视/深化时，`render(views=[...])` 只从 `baseAssetId` 继续 → 必要时 `revise_asset` 局部精修 → `task_complete` → Run 记录 step/tool/deliverable/status/隐藏模型 usage，全程流式回前端。
...
- **Run 记录**：每轮 `/api/agent` 创建 `run-...`，写到 `.data/projects/<id>/runs/<runId>.json`，项目状态保留最近 30 条摘要；`onStepFinish` 记录 step/tool 摘要，`render`/`revise_asset` 记录 Deliverable，`prompt-writer`/`inspect` 等隐藏模型调用也写 usage，`onFinish/onError/onAbort` 收口状态。
- **模型分档现状**：主脑默认 Sonnet 4.6 控成本；prompt-writer 默认 Opus 4.8 保留写图/世界知识优势；inspect 默认 Sonnet 4.6；成本解释走 DeepSeek V4 Flash。四者均可用 `RHEMOS_*_MODEL` 环境变量覆盖。
...
| `render` | **唯一生图入口**：给**中文意图**，内部 prompt-writer 写英文 prompt；final 模式硬要求 spec.identity + layout confirmed/skipped；按平面图出图时同时使用 reference PNG 和 **Rhemos CAD v1** 机读布局硬锁（坐标系/开口边/footprint/对象 bbox+layer+shape+height+facing+material）；默认输出 `candidate-set` 首稿候选，用户选 `baseAssetId` 后才允许 views 多视角深化；`autoCheck` 默认 false | gpt-image-2(fal) + **Opus prompt-writer** | intent / views / planAssetId / mode / n / quality / autoCheck → **Deliverable** |
...
| 判图 / 一致性检查 | 默认 `anthropic/claude-sonnet-4.6`，`RHEMOS_INSPECT_MODEL` 可覆盖 | Gateway；按需启用 |
...
src/
  ...
  agent/
    ...
    inspect.ts                     结构化判图 helper（generateObject + 默认 Sonnet；含分维度 dimensions + usage 追踪）
  ...
  lib/
    ...
    cad.ts                         Rhemos CAD v1：BoothLayout → 机读 CAD 文档 / 生图硬锁
