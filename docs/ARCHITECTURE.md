...
| `present_layout` | **方案定稿后推布局**：前端据此**自动弹布局编辑器**让用户精调 / 跳过 | — | intro / layout → 透传，前端弹 LayoutEditor |
| `render` | **唯一生图入口**：给**中文意图**，内部 prompt-writer 写英文 prompt；按 intent/views/planAssetId 自动选 单张 best-of-N / 进化链多视角 / 平面图条件化；identity·判图要点自读 spec | gpt-image-2 + Gemini + **Opus** | intent / views / planAssetId / n / quality → **Deliverable** |
