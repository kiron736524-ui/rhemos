...
模型层  src/models/gateway.ts + image-providers.ts —— 多来源：脑/判图/写prompt/语音清理/Gemini 经 Gateway · gpt-image-2 经 fal.ai · ASR 经 DashScope 直连
...
| `render` | **唯一生图入口**：给**中文意图**，内部 prompt-writer 写英文 prompt；final 模式硬要求 spec.identity + layout confirmed/skipped；按 intent/views/planAssetId 自动选 单张 best-of-N / 进化链多视角 / 平面图条件化 | gpt-image-2(fal) + Gemini(fallback) + **Opus** | intent / views / planAssetId / mode / n / quality → **Deliverable** |
...
| 文生图 | `openai/gpt-image-2` | **fal.ai**（`fal.run/openai/gpt-image-2`，`falTextToImage`）；默认 quality high 偏慢（fal API 速度 ≠ ChatGPT 体感）|
| 参考条件化 / 编辑 | `openai/gpt-image-2`（fal，优先）/ `google/gemini-3-pro-image`（fallback）| **fal `…/edit`**（`falEditFromRefs`，base64 data URI 多图参考）；失败回退 **Gemini 经 Gateway**（`generateText`+image）。〔历史 D27：gpt-image-2 经 Gateway 图输入不通——现走 fal，不再依赖 Gateway / OpenAI 直连〕|
...
> **多来源例外**：gpt-image-2 经 **fal.ai**、ASR 经 **DashScope** 直连，其余经 Gateway。`image-providers.ts` 把 `textToImage`/`editFromRefs` 收口为 provider 层——生产化时在一处切 OpenAI 直连 / Gateway / fal / Seedream，详见 [DECISIONS](DECISIONS.md) D29。
