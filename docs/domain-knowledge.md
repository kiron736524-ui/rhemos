...
**用法（关键）**：这不是问题脚本，是大脑的**内部 gap 分析表**。每轮 `read_project_state` 后，大脑对照本表判断"一个专业设计师此刻最该补哪条"；布局骨架问题**只问当前最高价值的 1 个缺口**，用户选完后再重排下一项。
...
- 布局骨架一次 1 问，每问 2-4 个**具体可视化**选项，**绝不开放式**；不生成"其他/自定义"（前端自动追加自由输入）。非布局信息也要克制，通常 0-2 个。
