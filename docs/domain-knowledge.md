...
2. **自检必须"融入第一次结果"，而非甩给用户。** 旧 `quality-check.ts` 在 2026-04 被废，根因不是"检查质量"错了，而是它把"半成品 + 改进报告"丢给用户、逼用户再点一次——违反第一性（用户发一次生图要的是成品，不是作业）。新架构的自检**100% 内部、对用户隐形**：用户看到的"第一次结果"本身就是已被大脑自我修正过的成品。三层落实（优先级从高到低）：
   - **① 预防（主力）**：大脑生图前把领域规则 + 锚定表 + 自检维度当成"写 prompt 的纪律清单"跑一遍，让第一张就对（= 旧团队"自检前移"的硬道理，零延迟零打扰）。
   - **② 静默择优（best-of-N）**：并行生 2-3 张，内部判图，只交最好的一张。监督=隐形筛选，并行几乎不增延迟。
   - **③ 有界静默修复**：仅当最好的一张仍有**客观结构性硬伤**（悬浮/乱码/比例崩）才做 1 次隐形 gpt-image-2 编辑再复检。口味问题不触发。
   - **红线**：分清**客观缺陷**（大脑隐形自动处理）与**主观口味**（走自然对话迭代，绝不进自检循环）。失败时静默交付最好结果、**不出报告**；只有遇到必须用户拍板的真实抉择才用一句自然问话开口。
   - **深度由大脑按"这次的分量"自定**：概念草图→预防+单张；最终交付→预防+择优+修复。
   - 用 Sonnet 4.6 vision 判图（远强于当年 gemini-1.5-flash），是这套隐形监督可行的技术前提，也是相对 rhemax 最硬的升级。
...
- **工作信条（替代旧 FSM）**：
  ...
  - **生成后必先自检**（调 `inspect_result`），发现偏差就自己写纠正 prompt 再 `revise_asset`——这是你的工作习惯，不是可选项。
...
## 4. 生图自检 Rubric（重组自 result-review + prompt-craft checklist + 废弃QC维度）

**三种用法（都对用户隐形，见 §0 判断 2）**：同一套维度，大脑在三个时机用——
1. **生成前 · prompt 纪律清单**（主力）：写 prompt 时逐维度自查，预防胜于纠正，零延迟零打扰。
2. **生成后 · 静默择优**：`inspect_result`（Sonnet 4.6 vision）对并行 best-of-N 各张按维度打分，选最优交付。
3. **静默修复门**：仅 **fail 级客观硬伤**触发 ≤1 次隐形 `revise_asset`；warning / 口味级**不触发**，留给自然对话。

判图永远是 **"输出 vs DesignSpec" 的客观对比**，返回 `{维度, pass/warning/fail, evidence}`——**只驱动内部动作，绝不生成用户报告**。下列维度均为**客观可判**项（主观口味不在此列）：
...
> **预防为主、纠正为辅**：同一套维度在"生成前"作为 prompt 纪律清单（主力、零延迟），在"生成后"作为静默择优与修复门（仅客观硬伤）。两道关都对用户隐形，用户只收到成品，绝不收到报告或被逼二次操作。
...
大脑在生图前写出的结构化方案，**一物三用**：给用户看的中文方案（narrative）、生图 prompt 来源（五层 layers）、自检基准（invariants + selfCheckTargets）。这让"用户只提交信息、大脑写方案并为结果负责"落地：spec 是被 `inspect_result` 拿来做"输出 vs spec"对比的硬基准。结构见 engineering-plan §4.2。
