...
## 质量闭环基础设施（2026-06-24 第七轮）
- **D31 · 质量闭环基础设施：单测 + 回归样例 + provider 可配置 + 生成耗时/供应商记录**。本轮不加产品能力，把"可维护工程原型"推进到"可回归验证"：① **booth-rules 单测**（Vitest，`src/lib/booth-rules.test.ts`）覆盖 12 类规则（合法/空zones/越界/接待堵口/四面开中心阻断/只家具/主视觉缺失/面积超额/关键区重叠/两面开关系/储物位/大件高柜）+ `openingRelation`；轻量修规则 6（`OPENING_RELATION_UNCLEAR` 原分支不可达——openingRelation 在 length===2 时永不返回 unknown——改为"两开口标到同一条边"触发；normalize 已去重，生产路径不误报、不破坏 present-layout/choices/render 接入）。② **真实案例回归样例** `fixtures/booth-cases/basic-cases.json`（10 个典型展台：9/18/36/72/100㎡，相邻/相对两面开、三面/四面开、大件产品、大 LED、无顶、suspended_truss）+ `src/lib/booth-cases.test.ts`：对每个 case 走 normalize → 跑纯函数规则（**不调模型、不需 key、不联网**），shouldFailRules=false→无 blocker、true→至少一条 fail/blocker，并校验 mustHaveZones 存在。③ **IMAGE_PROVIDER 可配置**（`src/models/image-providers.ts`）：默认 `fal`（当前唯一真实实现）；`getImageProviderName()` 对未知值**抛清晰错误**（不静默回退，免误判走了别的 provider）；openai/seedream/gemini 为预留接口，调用即抛"尚未实现"（**不伪造**）；新增 `.env.example`。④ **生成元数据沉淀**：`Asset` 加可选 `provider/model/quality/size/mode/durationMs`，`saveAsset` 支持；render（文生图/平面图/视角）与 revise 用 `Date.now()` 计批次墙钟、写进资产元数据并各补一条 run 事件——**fal 慢从体感变成可统计数据**，为日后对比 GPT Image 2 / Seedream / Gemini 留统一入口。`test` 脚本 = `vitest run`。Phase 5 仍含：DB、对象存储、队列、成本计费、取消/重试。落点：`src/lib/{booth-rules,booth-cases}.test.ts`、`fixtures/booth-cases/basic-cases.json`、`src/models/image-providers.ts`、`src/lib/{types,storage}.ts`、`src/tools/{render,revise-asset}.ts`、`vitest.config.ts`、`.env.example`、`package.json`。

## 生图输入快照（2026-06-24 第八轮）
- **D32 · RenderInputSnapshot：生图输入快照化**。附件资产化（D28）只解决"原始文件稳定保存"；本轮补"每次真正送进图像模型的**输入**稳定、可追踪、可复现"。每次 render / revise 在调用 provider **之前**写一条快照到 `.data/projects/<id>/render-inputs/<snapshotId>.json`（per-project lock、tombstone 不写、JSON 格式化），记录：mode、provider、model、quality、size、**最终 prompt**（已含 identity + render style anchor）、中文 intent、view、operation（text-to-image / plan-conditioned / view-generation / revision）、specSummary、layoutSummary、refs（plan→`floor_plan` / hero·已通过视角→`previous_render` / revise→parent）、ruleIssues（render 前 booth-rules 结果）。**provider 失败也留快照**（先写后调）。生成出的 `Asset` 关联 `renderInputId` + `sourceAssetIds`，据此可回答"这张图用了哪个 prompt / provider / 质量档、引用了哪些素材或历史图、基于哪个 spec / layout、生成前有哪些规则问题、能否复现/对比"。**不存图片 base64**、不替代 DB/对象存储、不反复喂回大脑——`read_project_state` 只回最多 5 条轻量摘要（id/mode/operation/provider/model/quality/size/refCount/createdAt），完整内容仅供开发者审计。数据链路：上传 → Attachment → Brief/Spec/Layout → **RenderInputSnapshot** → Provider → Asset（关联 renderInputId）→ Run。Phase 5 再迁 DB + 对象存储 + render_jobs。落点：`lib/{types,storage}.ts`、`tools/{render,revise-asset,read-project-state}.ts`、`lib/render-inputs.test.ts`。
