...
export const reviseAsset = tool({
  description:
    '窄回退修复（每资产最多 1 次）：仅对择优后仍存的**客观硬伤**做定向修正。你写一份"已修正的完整英文 prompt"（从对的部分采样、只改硬伤），重生一张并复检。返回新资产 + 复检结果。',
  ...
  execute: async ({ parentAssetId, correctedPrompt, criteria, quality, size }) => {
    ...
    const asset = await saveAsset(DEFAULT_PROJECT, bytes, {
      kind: 'booth-image',
      prompt: correctedPrompt,
