...
export const generateBestOfN = tool({
  description:
    '生图主力（横向 best-of-N）：用 gpt-image-2 并行生成 N 张候选（N≤2），并行客观判图，返回所有候选 + 推荐（最佳）。墙钟≈单张。拿到推荐图后你决定：直接交付 / 对它 revise / 重来。延迟：quality low~8s / medium~30s / high~200s——短期默认 high（质量优先），用户要快才 medium。',
  inputSchema: z.object({
    prompt: z.string().describe('英文五层架构 prompt（品牌占位）'),
    n: z.number().int().min(1).max(MAX_PARALLEL_IMAGES).default(1).describe(`并行候选数（≤${MAX_PARALLEL_IMAGES}）：概念用 1，要择优用 2`),
  ...
  execute: async ({ prompt, n, quality, size, criteria }, opts) => {
    ...
    for (const bytes of batches) {
      if (bytes) assets.push(await saveAsset(pid, bytes, { kind: 'booth-image', prompt }));
    }
